...
def delete_issued_books(request):
    if request.method=='POST':
        ...
        issuedbooks=models.IssuedBook.objects.filter(enrollment=enrollment,isbn=isbn)

        if issuedbooks is not None:
             issuedbooks.delete()
            #  return HttpResponseRedirect("issued books deleted successfully!")
             return redirect("/viewissuedbook")
        else:
            print("INVALID")
    return render(request,"library/delete_issued_books.html")  
